...
final class ProviderInstallerHermesTests: XCTestCase {
    func testInstallsHooksIntoRootAndNamedProfileConfigsWithoutRemovingForeignHooks() throws {
        ...
        let command = "/tmp/aegis-hermes-bridge"
    ...
    func testReplacesLegacyManagedHooksWithoutTouchingForeignHooks() throws {
        let hermesRoot = try makeHermesRoot()
        defer { try? FileManager.default.removeItem(at: hermesRoot) }
        let config = hermesRoot.appendingPathComponent("config.yaml")
        let legacyCommand = "/Users/test/.caesura-island/bin/caesura-island-hermes-bridge"
        try """
        model: test-model
        hooks:
          # caesura-island-managed
          pre_tool_call:
        ...
        let installed = try String(contentsOf: config, encoding: .utf8)
        XCTAssertFalse(installed.contains(legacyCommand))
        XCTAssertFalse(installed.contains("# caesura-island-managed"))
        XCTAssertTrue(installed.contains("/usr/local/bin/foreign-hook"))
        XCTAssertTrue(installed.contains(command))
    }
    ...
    private func makeHermesRoot() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("aegis-hermes-installer-tests-\(UUID().uuidString)", isDirectory: true)
