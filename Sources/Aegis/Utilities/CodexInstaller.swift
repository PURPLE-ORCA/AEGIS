...
/// Installs Aegis hooks into OpenAI Codex (`$CODEX_HOME/hooks.json`
/// + `config.toml`).
///
/// Safety contract:
/// - hooks.json: read existing, merge ONLY Aegis's own entries,
...
enum CodexInstaller {
    ...
    static func install() -> Bool {
        ...
        migrateLegacyRules(in: dir)
        installBridgeLauncher()

        print("[Aegis] Codex hooks installed at \(dir.path)")
        return hooksOK && tomlOK
    }

    // MARK: - hooks.json (merge, don't overwrite)

    /// Merges Aegis's hooks into an existing hooks.json. Foreign
    ...
    private static func writeHooksJSON(in codexDir: URL) -> Bool {
        ...
        // entry from older Aegis installs while preserving foreign data.
        if var notificationEntries = hooks["Notification"] as? [[String: Any]] {
            notificationEntries.removeAll(where: isAegisEntry)
            if notificationEntries.isEmpty { hooks.removeValue(forKey: "Notification") }
            else { hooks["Notification"] = notificationEntries }
        }
        for ev in events {
            mergeAegisEntry(in: &hooks, event: ev, command: bridgeCmd, timeout: 5)
        }
        ...
        mergeAegisEntry(in: &hooks, event: "SessionEnd", command: bridgeCmd, timeout: 3)
        for ev in permissionEvents {
            mergeAegisEntry(in: &hooks, event: ev, command: bridgeCmd, timeout: 300)
        }
        root["hooks"] = hooks
    ...
    /// Inserts or updates Aegis's entry for `event` while leaving any
    /// foreign entries (user customizations, other tools) intact.
    private static func mergeAegisEntry(in hooks: inout [String: Any], event: String, command: String, timeout: Int) {
        ...
        // Remove ALL existing Aegis entries (not just the first) before
        ...
        entries.removeAll(where: isAegisEntry)
        entries.append(ourEntry)
        hooks[event] = entries
    }

    private static func isAegisEntry(_ entry: [String: Any]) -> Bool {
        guard let inner = entry["hooks"] as? [[String: Any]] else { return false }
        return inner.contains {
            guard let command = $0["command"] as? String else { return false }
            return command.contains("aegis") || command.contains("caesura-island")
        }
    }
    ...
    private static func migrateLegacyRules(in codexDir: URL) {
        let rulesDir = codexDir.appendingPathComponent("rules", isDirectory: true)
        let legacy = rulesDir.appendingPathComponent("caesuraisland.rules")
        let current = rulesDir.appendingPathComponent("aegis.rules")
        guard FileManager.default.fileExists(atPath: legacy.path),
              let legacyText = try? String(contentsOf: legacy, encoding: .utf8) else { return }

        let currentText = (try? String(contentsOf: current, encoding: .utf8)) ?? ""
        let separator = currentText.isEmpty || currentText.hasSuffix("\n") ? "" : "\n"
        do {
            try (currentText + separator + legacyText).write(to: current, atomically: true, encoding: .utf8)
            try FileManager.default.removeItem(at: legacy)
        } catch {
            Log.error("CodexInstaller: failed to migrate legacy approval rules: \(error)")
        }
    }

    private static func bridgeCommand() -> String {
        let home = FileManager.default.homeDirectoryForCurrentUser
        return "\(home.path)/.aegis/bin/aegis-codex-bridge"
    }
    ...
    private static func installBridgeLauncher() {
        ...
        let launcher = binDir.appendingPathComponent("aegis-codex-bridge")
        ...
        let script = [
            "#!/bin/zsh",
            "# aegis-codex-bridge launcher (auto-generated by Aegis)",
            "H=/Contents/Helpers/AegisBridge",
            "B=\(quotedBundledBridge); [ -x \"$B\" ] && exec \"$B\" --source codex \"$@\"",
            ...
            "[ -x \"$D\" ] && exec \"$D\" --source codex \"$@\"",
            "echo \"aegis-codex-bridge: app not found.\" >&2",
            "exit 127",
        ].joined(separator: "\n")
