...
/// to `~/.codex/rules/aegis.rules`. Codex consults this file on every
...
enum CodexPermissionRules {
    ...
    static func persistAlwaysAllow(toolName: String, toolInput: String?, broad: Bool = false) -> Bool {
        ...
        let rulesFile = rulesDir.appendingPathComponent("aegis.rules")
        do {
    ...
    private static func ruleBlock(for pattern: [String], broad: Bool) -> String {
        ...
        return """
        # Added by Aegis when "\(label)" was clicked for Codex.
        prefix_rule(
            \(patternLineFor(pattern: pattern)),
            decision = "allow",
            justification = "Allowed from Aegis \(label)",
        )
