...
final class SocketServer {
    private let socketPath = "/tmp/aegis.sock"
    private let acceptQueue = DispatchQueue(label: "dev.aegis.socket.accept", qos: .userInitiated)
    private let clientQueue = DispatchQueue(label: "dev.aegis.socket.client", qos: .userInitiated, attributes: .concurrent)
    ...
    func start() {
        ...
        guard serverFd >= 0 else {
            print("[Aegis] Failed to create socket: \(String(cString: strerror(errno)))")
            return
        }
        ...
        guard bindResult == 0 else {
            print("[Aegis] Failed to bind socket: \(String(cString: strerror(errno)))")
            close(serverFd)
            serverFd = -1
            return
        }
        ...
        guard listen(serverFd, 16) == 0 else {
            print("[Aegis] Failed to listen: \(String(cString: strerror(errno)))")
            close(serverFd)
            serverFd = -1
            return
        }
    ...
    func stop() {
        ...
        unlink(socketPath)
        print("[Aegis] Socket server stopped")
    }
