...
enum NotchThemeID: String, CaseIterable, Identifiable {
    ...
    var blurb: String {
        switch self {
        case .aegis:    return "Near-black surfaces, restrained violet edges, and clean macOS typography."
...
extension NotchTheme {
    ...
    static let aegisTheme = NotchTheme(
        id: .aegis,
...
private struct NotchThemeKey: EnvironmentKey {
    static let defaultValue: NotchTheme = .aegisTheme
}
...
extension View {
    ...
    func notchCard(_ theme: NotchTheme, tint: Color? = nil, active: Bool = false,
        ...
        let fill: Color = ink?.fill
            ?? (theme.id == .aegis ? theme.neutralCardFill : nil)
    ...
    func notchPill(_ theme: NotchTheme, fill: Color, stroke: Color? = nil, base: CGFloat = 5) -> some View {
        ...
        let resolvedFill = theme.id == .aegis ? theme.boxFill : fill
        let resolvedStroke = theme.id == .aegis ? (stroke ?? theme.boxStroke) : stroke
        return self
            .background(
