...
final class SoundEngine {
    ...
    private func ensureAudioEngineRunning() -> (player: AVAudioPlayerNode, generation: Int)? {
        ...
        do {
            ...
            print("[Aegis] Audio engine failed to start: \(error)")
            return nil
        }
    }
    ...
    private func loadAudioFile(_ url: URL) -> AVAudioPCMBuffer? {
        ...
        do { try file.read(into: srcBuffer) } catch {
            print("[Aegis] Failed to read audio file \(url.lastPathComponent): \(error)")
            return nil
        }
        ...
        if let error {
            print("[Aegis] Audio convert failed: \(error.localizedDescription)")
            return srcBuffer
        }
        return outBuffer
    }
