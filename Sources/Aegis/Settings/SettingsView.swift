...
struct SettingsView: View {
    ...
    /// `.build/debug/Aegis` (no Info.plist version baked in).
    ...
    /// Loads the packaged app icon when available. Debug builds fall back to
    /// the neutral gradient mark below.
        ...
        Section {
            Toggle(isOn: $settingsStore.companionEnabled) {
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Show Mysa")
                        Text("A calm Aegis sentinel for agent state and attention.")
                            .font(.system(size: 11))
                            .foregroundColor(.secondary)
                    }
                }
    ...
    private var integrationsForm: some View {
        Section {
            ...
            Text("Hooks auto-install every time Aegis launches. Use these buttons only if you need to refresh or re-pair after deleting config files.")
                .font(.system(size: 11))
                .foregroundColor(.secondary)
                .multilineTextAlignment(.leading)
                .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private var aboutForm: some View {
        ...
        Section {
            ...
            Text("Aegis runs from the menu bar (and the notch). Quitting stops session tracking until you reopen it.")
                .font(.system(size: 11))
                .foregroundColor(.secondary)
                .multilineTextAlignment(.leading)
                .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private func openSoundPacksFolder() {
        ...
        let dir = home.appendingPathComponent(".aegis/sound-packs")
