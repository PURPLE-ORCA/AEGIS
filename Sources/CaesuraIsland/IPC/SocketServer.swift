...
final class SocketServer {
    private let socketPath = "/tmp/caesura-island.sock"
    private let acceptQueue = DispatchQueue(label: "dev.caesuraisland.socket.accept", qos: .userInitiated)
    private let clientQueue = DispatchQueue(label: "dev.caesuraisland.socket.client", qos: .userInitiated, attributes: .concurrent)
    ...
    func start() {
        ...
        guard serverFd >= 0 else {
            print("[CaesuraIsland] Failed to create socket: \(String(cString: strerror(errno)))")
            return
        }
        ...
        guard bindResult == 0 else {
            print("[CaesuraIsland] Failed to bind socket: \(String(cString: strerror(errno)))")
            close(serverFd)
            serverFd = -1
            return
        }
        ...
        guard listen(serverFd, 16) == 0 else {
            print("[CaesuraIsland] Failed to listen: \(String(cString: strerror(errno)))")
            close(serverFd)
            serverFd = -1
            return
        }
    ...
    func stop() {
        ...
        unlink(socketPath)
        print("[CaesuraIsland] Socket server stopped")
    }
    ...
    private func handleClient(_ fd: Int32) {
        ...
        Log.info("Socket: parsed message source=\(message.source ?? "nil") hookEvent=\(message.hookEvent) session=\(message.sessionId.prefix(12)) toolName=\(message.toolName ?? "nil") effort=\(message.effortLevel ?? "nil") durationMs=\(message.durationMs.map(String.init) ?? "nil") userMessage=\(message.userMessage?.prefix(40) ?? "nil")")
