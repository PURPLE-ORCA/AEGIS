...
struct SettingsView: View {
    ...
    /// `.build/debug/CaesuraIsland` (no Info.plist version baked in).
    ...
    private var integrationsForm: some View {
        Section {
            ...
            Text("Hooks auto-install every time CAESURA-ISLAND launches. Use these buttons only if you need to refresh or re-pair after deleting config files.")
                .font(.system(size: 11))
                .foregroundColor(.secondary)
                .multilineTextAlignment(.leading)
                .frame(maxWidth: .infinity, alignment: .leading)
        ...
        Section {
            Button(role: .destructive) {
                ...
                HStack {
                    ...
                    Text("Quit CAESURA-ISLAND")
                }
            }
        } footer: {
            Text("CAESURA-ISLAND runs from the menu bar (and the notch). Quitting stops session tracking until you reopen it.")
                .font(.system(size: 11))
                .foregroundColor(.secondary)
                .multilineTextAlignment(.leading)
                .frame(maxWidth: .infinity, alignment: .leading)
