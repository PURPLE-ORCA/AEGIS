...
final class SettingsStore: ObservableObject {
    ...
    /// UserDefaults AND mirrored to ~/.caesura-island/config.json so the bridge
    ...
    /// event-named files), living in `~/.caesura-island/sound-packs/`.
    func soundLibraryDirectory() -> URL {
        let dir = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(".caesura-island/sound-packs")
    ...
    /// Writes ~/.caesura-island/config.json — the bridge reads this each run to
    /// decide whether to block a provider's `before*` hook for approval.
    private func writeBridgeConfig() {
        let dir = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(".caesura-island")
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    private func updateLoginItem() {
        ...
        do {
            ...
            print("[CaesuraIsland] Login item error: \(error)")
        }
    }
}
