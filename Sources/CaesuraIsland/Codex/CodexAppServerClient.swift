...
/// `thread/started` notifications so CAESURA-ISLAND can pick up the user-renamed
...
final class CodexAppServerClient: ObservableObject {
    ...
    private func spawn() {
        ...
        NSLog("[CaesuraIsland] Spawning codex app-server from \(executablePath)")
        generation += 1
        let gen = generation
        lastSpawnAt = Date()
        ...
        do {
            ...
            NSLog("[CaesuraIsland] Failed to spawn codex app-server: \(error.localizedDescription)")
        }
    }
    ...
    private func handleTermination(gen: Int) {
        ...
        if consecutiveQuickFailures >= maxConsecutiveQuickFailures {
            NSLog("[CaesuraIsland] codex app-server has died %d times in a row — giving up", consecutiveQuickFailures)
            return
        }

        let delaySeconds = min(pow(2.0, Double(consecutiveQuickFailures)) * 5, 300)
        NSLog("[CaesuraIsland] codex app-server died (lifetime=%.1fs); relaunching in %.0fs (failure #%d)", lifetime, delaySeconds, consecutiveQuickFailures)
        relaunchTask?.cancel()
    ...
    private func handleLine(_ line: Data) {
        ...
        case "thread/closed", "thread/archived":
            ...
            if let id = closedId {
                ...
                NSLog("[CaesuraIsland] Codex thread closed: \(id)")
            }
        default:
            break
        }
