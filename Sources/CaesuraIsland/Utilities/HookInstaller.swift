...
/// Installs CAESURA-ISLAND hooks into Claude Code's settings.json.
///
/// Safety contract:
...
enum HookInstaller {
    static let bridgePath = "~/.caesura-island/bin/caesura-island-bridge"
    static let statusLinePath = "~/.caesura-island/bin/caesura-island-statusline"
    ...
    private static func installHooks(intoClaudeDir claudeDir: URL) -> Bool {
        ...
        let bridgeCommand = FileManager.default.homeDirectoryForCurrentUser.path + "/.caesura-island/bin/caesura-island-bridge"
        ...
        print("[CaesuraIsland] Hooks installed at \(settingsPath.path)")
        return true
    }
    ...
    private static func addHookEntry(to hooks: inout [String: Any], event: String, command: String, timeout: Int?) {
        ...
        // Remove any existing CAESURA-ISLAND entries (including duplicates that
        ...
        let isOurs: ([String: Any]) -> Bool = { entry in
            ...
            return entryHooks.contains { ($0["command"] as? String)?.contains("caesura-island") == true }
        }
    ...
    private static func installBridgeLauncher() {
        ...
        let bridgePath = binDir.appendingPathComponent("caesura-island-bridge")

        let script = [
            "#!/bin/zsh",
            "# caesura-island-bridge launcher (auto-generated by CAESURA-ISLAND)",
            ...
            "[ -x \"$D\" ] && exec \"$D\" \"$@\"",
            "echo \"caesura-island-bridge: app not found.\" >&2",
            "exit 127",
        ].joined(separator: "\n")
    ...
    private static func installStatusLine() {
        ...
        let binDir = home.appendingPathComponent(".caesura-island/bin")
        try? FileManager.default.createDirectory(at: binDir, withIntermediateDirectories: true)
        let path = binDir.appendingPathComponent("caesura-island-statusline")

        let script = """
        #!/bin/bash
        # Claude Code StatusLine Script
        # Auto-configured by CAESURA-ISLAND
        input=$(cat)

        # Cache rate limits
        _rl=$(echo "$input" | jq -c '.rate_limits // empty' 2>/dev/null)
        [ -n "$_rl" ] && echo "$_rl" > "\(home.path)/.caesura-island/cache/rl.json"
        """
