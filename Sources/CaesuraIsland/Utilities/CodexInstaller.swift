...
/// Installs CAESURA-ISLAND hooks into OpenAI Codex (`$CODEX_HOME/hooks.json`
/// + `config.toml`).
///
/// Safety contract:
/// - hooks.json: read existing, merge ONLY CAESURA-ISLAND's own entries,
...
enum CodexInstaller {
    ...
    static func install() -> Bool {
        ...
        print("[CaesuraIsland] Codex hooks installed at \(dir.path)")
        return hooksOK && tomlOK
    }

    // MARK: - hooks.json (merge, don't overwrite)

    /// Merges CAESURA-ISLAND's hooks into an existing hooks.json. Foreign
    ...
    /// Inserts or updates CAESURA-ISLAND's entry for `event` while leaving any
    ...
    private static func mergeCaesuraIslandEntry(in hooks: inout [String: Any], event: String, command: String, timeout: Int) {
        ...
        // Remove ALL existing CAESURA-ISLAND entries (not just the first) before
    ...
    private static func bridgeCommand() -> String {
        let home = FileManager.default.homeDirectoryForCurrentUser
        return "\(home.path)/.caesura-island/bin/caesura-island-codex-bridge"
    }
    ...
    private static func installBridgeLauncher() {
        ...
        let launcher = binDir.appendingPathComponent("caesura-island-codex-bridge")
        let bundledBridge = Bundle.main.bundleURL
            .appendingPathComponent("Contents/Helpers/CaesuraIslandBridge").path
        let quotedBundledBridge = "'" + bundledBridge.replacingOccurrences(of: "'", with: "'\\''") + "'"

        let script = [
            "#!/bin/zsh",
            "# caesura-island-codex-bridge launcher (auto-generated by CAESURA-ISLAND)",
            ...
            "[ -x \"$D\" ] && exec \"$D\" --source codex \"$@\"",
            "echo \"caesura-island-codex-bridge: app not found.\" >&2",
            "exit 127",
        ].joined(separator: "\n")
