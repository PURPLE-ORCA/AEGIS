...
sample_count="${1:-12}"
caesura_pid="${2:-$(pgrep -f '/build/CAESURA-ISLAND.app/Contents/MacOS/CaesuraIsland' | head -1)}"
history_root="$HOME/.codex/sessions"

if [[ -z "$caesura_pid" ]]; then
    print -u2 "CAESURA-ISLAND is not running from the workspace app bundle."
    exit 1
fi
...
lsof_output=$(lsof -p "$caesura_pid" 2>/dev/null)
...
resident_kib=$(ps -p "$caesura_pid" -o rss= | tr -d ' ')

cpu_samples=$(
    top -l "$sample_count" -s 1 -pid "$caesura_pid" -stats pid,cpu,mem,threads,time,command |
        awk -v pid="$caesura_pid" '$1 == pid { print $2 }'
)
average_cpu=$(print -r -- "$cpu_samples" | awk 'NR > 1 { sum += $1; count++ } END { if (count > 0) printf "%.2f", sum / count; else print "n/a" }')

print "pid=$caesura_pid"
